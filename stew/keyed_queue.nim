...
func shiftImpl[K, V](rq: var KeyedQueue[K, V]) =
...
func popImpl[K, V](rq: var KeyedQueue[K, V]) =
  ## Expects: rq.tab.len != 0
...
func deleteImpl[K, V](rq: var KeyedQueue[K, V], key: K) =
...
func appendImpl[K, V](rq: var KeyedQueue[K, V], key: K, val: V) =
...
func prependImpl[K, V](rq: var KeyedQueue[K, V], key: K, val: V) =
  ## Expects: not rq.tab.hasKey(key)
...
func shiftKeyImpl[K, V](rq: var KeyedQueue[K, V]): Opt[K] =
  if 0 < rq.tab.len:
    let key = rq.kFirst
    rq.shiftImpl
    Opt.some(key)
...
func popKeyImpl[K, V](rq: var KeyedQueue[K, V]): Opt[K] =
  if 0 < rq.tab.len:
    let key = rq.kLast
    rq.popImpl
    Opt.some(key)
...
func firstKeyImpl[K, V](rq: var KeyedQueue[K, V]): Opt[K] =
  if rq.tab.len == 0:
    Opt.none(K)
  else:
    Opt.some(rq.kFirst)

func secondKeyImpl[K, V](rq: var KeyedQueue[K, V]): Opt[K] =
  if rq.tab.len < 2:
    return Opt.none(K)
  noKeyError("secondKeyImpl"):
    return Opt.some(rq.tab[rq.kFirst].kNxt)

func beforeLastKeyImpl[K, V](rq: var KeyedQueue[K, V]): Opt[K] =
  if rq.tab.len < 2:
    return Opt.none(K)
  noKeyError("lastKeyImpl"):
    return Opt.some(rq.tab[rq.kLast].kPrv)

func lastKeyImpl[K, V](rq: var KeyedQueue[K, V]): Opt[K] =
  if rq.tab.len == 0:
    Opt.none(K)
  else:
    Opt.some(rq.kLast)

func nextKeyImpl[K, V](rq: var KeyedQueue[K, V], key: K): Opt[K] =
  if not rq.tab.hasKey(key) or rq.kLast == key:
    return Opt.none(K)
  noKeyError("nextKeyImpl"):
    return Opt.some(rq.tab[key].kNxt)

func prevKeyImpl[K, V](rq: var KeyedQueue[K, V], key: K): Opt[K] =
...
func init*[K, V](rq: var KeyedQueue[K, V], initSize = defaultInitialSize) =
...
func init*[K, V](T: type KeyedQueue[K, V], initSize = defaultInitialSize): T =
  ## Initaliser variant.
  result.init(initSize)

func init*[K](rq: var KeyedQueueNV[K], initSize = defaultInitialSize) =
  ## Key-only queue, no explicit values
  rq.tab = initTable[K, KeyedQueueItem[K, BlindValue]](initSize)

func init*[K](T: type KeyedQueueNV[K], initSize = defaultInitialSize): T =
  ## Initaliser variant.
  result.init(initSize)
...
func append*[K, V](rq: var KeyedQueue[K, V], key: K, val: V): bool =
...
func replace*[K, V](rq: var KeyedQueue[K, V], key: K, val: V): bool =
...
func `[]=`*[K, V](rq: var KeyedQueue[K, V], key: K, val: V) =
...
func prepend*[K, V](rq: var KeyedQueue[K, V], key: K, val: V): bool =
...
func shift*[K, V](rq: var KeyedQueue[K, V]): Opt[KeyedQueuePair[K, V]] =
...
func shiftKey*[K, V](rq: var KeyedQueue[K, V]): Opt[K] =
  ## Similar to `shift()` but with different return value.
  rq.shiftKeyImpl

func shiftValue*[K, V](rq: var KeyedQueue[K, V]): Opt[V] =
  ## Similar to `shift()` but with different return value.
  if 0 < rq.tab.len:
    rq.tab.withValue(rq.kFirst, item):
...
func pop*[K, V](rq: var KeyedQueue[K, V]): Opt[KeyedQueuePair[K, V]] =
...
func popKey*[K, V](rq: var KeyedQueue[K, V]): Opt[K] =
  ## Similar to `pop()` but with different return value.
  rq.popKeyImpl

func popValue*[K, V](rq: var KeyedQueue[K, V]): Opt[V] =
  ## Similar to `pop()` but with different return value.
  if 0 < rq.tab.len:
    rq.tab.withValue(rq.kLast, item):
      let res = Opt.some move(item[].data)
...
func delete*[K, V](rq: var KeyedQueue[K, V], key: K): Opt[KeyedQueuePair[K, V]] =
...
func del*[K, V](rq: var KeyedQueue[K, V], key: K) =
...
func append*[K](rq: var KeyedQueueNV[K], key: K): bool =
...
func prepend*[K](rq: var KeyedQueueNV[K], key: K): bool =
  ## Key-only queue variant
...
func shift*[K](rq: var KeyedQueueNV[K]): Opt[K] =
  ## Key-only queue variant
  rq.shiftKeyImpl

func pop*[K](rq: var KeyedQueueNV[K]): Opt[K] =
...
func hasKey*[K, V](rq: var KeyedQueue[K, V], key: K): bool =
  ## Check whether the argument `key` has been queued, already
  rq.tab.hasKey(key)

func eq*[K, V](rq: var KeyedQueue[K, V], key: K): Opt[V] =
...
func `[]`*[K, V](rq: var KeyedQueue[K, V], key: K): var V {.raises: [KeyError].} =
...
func lruFetch*[K, V](rq: var KeyedQueue[K, V], key: K): Opt[V] =
...
func lruUpdate*[K, V](rq: var KeyedQueue[K, V], key: K, val: V): bool =
...
func lruAppend*[K, V](rq: var KeyedQueue[K, V], key: K, val: V, maxItems: int): V =
...
func firstKey*[K, V](rq: var KeyedQueue[K, V]): Opt[K] =
...
func secondKey*[K, V](rq: var KeyedQueue[K, V]): Opt[K] =
...
func beforeLastKey*[K, V](rq: var KeyedQueue[K, V]): Opt[K] =
...
func lastKey*[K, V](rq: var KeyedQueue[K, V]): Opt[K] =
...
func nextKey*[K, V](rq: var KeyedQueue[K, V], key: K): Opt[K] =
...
func prevKey*[K, V](rq: var KeyedQueue[K, V], key: K): Opt[K] =
...
func first*[K, V](rq: var KeyedQueue[K, V]): Opt[KeyedQueuePair[K, V]] =
...
func second*[K, V](rq: var KeyedQueue[K, V]): Opt[KeyedQueuePair[K, V]] =
...
func beforeLast*[K, V](rq: var KeyedQueue[K, V]): Opt[KeyedQueuePair[K, V]] =
...
func last*[K, V](rq: var KeyedQueue[K, V]): Opt[KeyedQueuePair[K, V]] =
...
func next*[K, V](rq: var KeyedQueue[K, V], key: K): Opt[KeyedQueuePair[K, V]] =
...
func prev*[K, V](rq: var KeyedQueue[K, V], key: K): Opt[KeyedQueuePair[K, V]] =
...
func first*[K](rq: var KeyedQueueNV[K]): Opt[K] =
  ## Key-only queue variant
  rq.firstKeyImpl

func second*[K](rq: var KeyedQueueNV[K]): Opt[K] =
  ## Key-only queue variant
  rq.secondKeyImpl

func beforeLast*[K](rq: var KeyedQueueNV[K]): Opt[K] =
  ## Key-only queue variant
  rq.beforeLastKeyImpl

func last*[K](rq: var KeyedQueueNV[K]): Opt[K] =
  ## Key-only queue variant
  rq.lastKeyImpl

func next*[K](rq: var KeyedQueueNV[K], key: K): Opt[K] =
  ## Key-only queue variant
  rq.nextKeyImpl(key)

func prev*[K](rq: var KeyedQueueNV[K], key: K): Opt[K] =
  ## Key-only queue variant
  rq.nextKeyImpl(key)
...
func firstValue*[K, V](rq: var KeyedQueue[K, V]): Opt[V] =
...
func secondValue*[K, V](rq: var KeyedQueue[K, V]): Opt[V] =
...
func beforeLastValue*[K, V](rq: var KeyedQueue[K, V]): Opt[V] =
...
func lastValue*[K, V](rq: var KeyedQueue[K, V]): Opt[V] =
...
func `==`*[K, V](a, b: var KeyedQueue[K, V]): bool =
...
func key*[K, V](kqp: KeyedQueuePair[K, V]): K =
  ## Getter
  kqp.key

func len*[K, V](rq: var KeyedQueue[K, V]): int =
  ## Returns the number of items in the queue
  rq.tab.len

func clear*[K, V](rq: var KeyedQueue[K, V]) =
  ## Clear the queue
  rq.tab.clear
  rq.kFirst.reset
  rq.kLast.reset

func toKeyedQueueResult*[K, V](key: K, data: V): Opt[KeyedQueuePair[K, V]] =
